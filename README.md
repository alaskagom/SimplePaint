...
## 실행화면(과제4)
-과제4코드의실행스크린샷
![과제4 실행화면](img/capture9-4.png)
![과제4 실행화면](img/capture9-5.png)


-과제내용  
 과제4는 외부 이미지 파일을 불러와 캔버스로 사용하고, 그 이미지 위에 도형을 그린 뒤 다시 파일로 저장하는 기능을 구현하는 것이다.


-구현내용과 기능설명  
OpenFileDialog를 사용하여 외부 이미지 파일을 선택할 수 있도록 구현하였다.  
선택한 이미지를 Bitmap으로 불러와 PictureBox에 표시하였다.  
불러온 이미지 위에 직선, 사각형, 원을 그릴 수 있도록 기존 그리기 기능과 연결하였다.  
이미지 크기에 맞게 캔버스 크기를 조정하였다.  
큰 이미지를 불러왔을 때 화면에서 확인할 수 있도록 스크롤 기능을 적용하였다.  
완성된 이미지를 저장할 수 있도록 기존 저장 기능과 연동하였다.  

사용자가 열기 버튼을 클릭하면 OpenFileDialog 창이 나타나고, 원하는 이미지 파일을 선택할 수 있다.  
선택된 이미지는 캔버스에 표시되며, 사용자는 그 위에 도형을 추가로 그릴 수 있다.  이미지가 큰 경우에는 스크롤을 이용하여 전체 이미지를 확인할 수 있고, 편집이 끝난 후 저장 버튼을 눌러 수정된 이미지를 파일로 저장할 수 있다.  